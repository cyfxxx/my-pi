...
│   │   ├── ui-adapter.ts             # 命令/快捷键/渲染器适配
│   │   └── session-adapter.ts        # 会话枚举适配
│   │
...
│   └── memory/                       # 自定义功能数据（memory 笔记 + 工具输出归档）
│
...
| 记忆/笔记数据 | `portable/memory/` | `PI_MEMORY_DIR`（由 `custom/core/config.ts` 的 `getMemoryDir` 识别） |
