/**
 * Plan-mode Feature — 任务状态机（纯逻辑，零 Pi 依赖）
 * 迁移自 pi-tools `agent/extensions/plan-mode/state.ts`。
 */
...
export function applyTaskMutation(state: TaskState, action: TaskAction, params: TaskMutationParams): ApplyResult {
  switch (action) {
    ...
    case 'update': {
      ...
      if (params.failure) {
        updated.failures = [...(current.failures ?? []), params.failure].slice(-3);
      }
      ...
      return {
        ...
        op: { kind: 'update', id: updated.id, fromStatus: current.status, toStatus: newStatus, failure: params.failure },
      };
    }
