...
const AGENT_DIR = process.env.PI_CODING_AGENT_DIR || join(process.env.HOME || '~', '.pi')
...
/**
 * 简化的 session 解析：从 JSONL 首行提取 session header
 * 替代 parseSessionEntries（不再依赖 @earendil-works/pi-coding-agent）
 */
function parseSessionHeader(line: string): { type: string; id?: string; timestamp?: string } | null {
  try {
    const obj = JSON.parse(line)
    if (obj && typeof obj === 'object' && obj.type === 'session') {
      return { type: 'session', id: obj.id, timestamp: obj.timestamp }
    }
    return null
  } catch {
    return null
...
function readFirstLine(filePath: string): string {
  try {
    ...
    try {
      // parseSessionEntries 返回 FileEntry[]（SessionHeader | SessionEntry），
      // 首条恒为 session header，按 type === 'session' 收窄即可
