...
// ── autopilot config（portable/agent/autopilot/config.json）──

/** 功能数据目录：autopilot 的配置/状态收敛到 portable/agent/autopilot/ */
function autopilotDir(): string {
  return path.join(getAgentDir(), 'autopilot');
}

/** 新路径优先，旧路径（agent 根）作为迁移回退。 */
function candidatePaths(envVar: string | undefined, name: string, legacy: string): string[] {
  if (envVar) return [envVar];
  return [path.join(autopilotDir(), name), path.join(getAgentDir(), legacy)];
}
...
function stateCandidates(): string[] {
  const env = process.env.PI_ADMIN_STATE_FILE;
  if (env) return [path.resolve(env)];
  return [path.join(autopilotDir(), 'state.json'), path.join(getAgentDir(), '.pi-admin-state.json')];
}
...
export function readState(): AdminState {
  for (const p of stateCandidates()) {
    try {
      return { ...defaultState(), ...(JSON.parse(fs.readFileSync(p, 'utf-8')) as Partial<AdminState>) };
    } catch {
      /* 试下一个候选路径 */
    }
  }
