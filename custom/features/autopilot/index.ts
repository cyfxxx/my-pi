 ...
 * autopilot 自动化特性
 *
 * 定时任务调度 + Agent 自管理 + 失败自愈（failover/看门狗/遥测/预算）
 */
...
export function init(pi: EnhancedExtensionAPI): void {
  scheduler = new SessionScheduler(pi as any)
  notified = false
  requeued = false
  lockAcquired = false
  ...
  registerCommands(pi as any, scheduler!)
