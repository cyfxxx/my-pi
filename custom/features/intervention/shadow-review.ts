 ...
 * 初期只记录 warning，待准确率达标后可升级为 block action。
 *
 * 使用方式：
 *   import { checkToolCall } from './shadow-review'
 *   const findings = checkToolCall('bash', { command: 'rm -rf /tmp/test' });
 *   if (findings.length > 0) recordFinding(findings[0]);
 *
 * 数据落盘：PI_MEMORY_DIR/stats/shadow-review.jsonl（仅记录，不注入上下文）。
 * 迁移自 pi-tools `agent/services/shadow-review.ts`（原落盘路径 ~/.pi/agent/stats）。
 */
...
/** 落盘文件：`PI_SHADOW_REVIEW_FILE` 可覆盖（便于测试隔离与运行时重定向） */
export function shadowReviewFile(): string {
  return process.env.PI_SHADOW_REVIEW_FILE ?? join(getMemoryDir(), 'stats', 'shadow-review.jsonl');
}
const MAX_DAYS = 30;

export function recordFinding(f: ShadowFinding): void {
  try {
...
const rules: ShadowRule[] = [
  // bash 危险命令
  {
    ...
    test: (input) => {
      ...
      if (/\brm\s+-[rRf]/.test(cmd))
        return {
          type: 'dangerous-command',
          severity: 'warning',
          tool: 'bash',
          ruleId: 'bash-dangerous-cmd',
          message: 'rm -rf 命令',
          match: cmd.slice(0, 100),
        };
      if (/\bsudo\b/.test(cmd))
        return {
          type: 'dangerous-command',
  ...
  // write/edit 敏感文件
  {
    ...
    test: (input) => {
      ...
      if (
        /auth\.json|settings\.json|models\.json|\.env|private[_-]?key/i.test(
          target,
        )
      ) {
        return {
    ...
    test: (input) => {
      ...
      if (content.length > 100_000) {
        return {
          ...
          tool: 'write',
          ruleId: 'large-write',
          message: `写入内容较大（${Math.round(content.length / 1024)}KB）`,
          match: `${content.length} chars`,
        };
      }
      return null;
    },
...
export function loadShadowFindings(maxDays = MAX_DAYS): ShadowFinding[] {
  try {
    ...
    return readFileSync(file, 'utf-8')
      ...
      .filter(
        (r): r is ShadowFinding & { ts: number } =>
          r !== null && r.ts >= cutoff,
      );
  } catch {
    return [];
 ...
 */
export function shadowReviewReport(
  maxDays = 7,
): Record<string, { count: number; severity: string }> {
