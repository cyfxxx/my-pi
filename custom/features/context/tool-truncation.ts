...
// Local type definitions (decoupled from pi-coding-agent)
...
interface TruncateResult { content: string; outputBytes: number; }
function truncateHead(text: string, opts: { maxBytes: number }): TruncateResult {
  const buf = Buffer.from(text, 'utf8');
  if (buf.length <= opts.maxBytes) return { content: text, outputBytes: buf.length };
  const truncated = buf.slice(buf.length - opts.maxBytes).toString('utf8');
  return { content: truncated, outputBytes: Buffer.byteLength(truncated, 'utf8') };
}
function truncateTail(text: string, opts: { maxBytes: number }): TruncateResult {
  const buf = Buffer.from(text, 'utf8');
  if (buf.length <= opts.maxBytes) return { content: text, outputBytes: buf.length };
  const truncated = buf.slice(0, opts.maxBytes).toString('utf8');
  return { content: truncated, outputBytes: Buffer.byteLength(truncated, 'utf8') };
}
