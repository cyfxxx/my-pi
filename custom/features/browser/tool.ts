/**
 * tool.ts - Browser tool registration
 *
 * Registers all browser tools using adapters
 */
import type { BrowserManager } from './browser/impl'
import type { RecordUsage } from './browser/helpers'
import { truncate, toolResult, createRequirePage } from './browser/helpers'
...
export interface BrowserTool {
  ...
  execute: (params: Record<string, unknown>, signal?: AbortSignal) => Promise<{ content: Array<{ type: 'text'; text: string }>; details: Record<string, unknown> }>
}

export function getBrowserTools(browser: BrowserManager, recordUsage: RecordUsage, viewportHeight: number = 800): BrowserTool[] {
  ...
  tools.push({
    ...
    label: '打开网页',
    description: '使用 CloakBrowser 隐身浏览器打开指定 URL，自动绕过反爬虫检测。返回页面标题、URL 和结构化摘要而非全文，大幅节省上下文。使用 browser_extract 获取完整文本。',
  ...
  tools.push({
    ...
    label: '截图',
    description: '截取当前浏览器页面的截图。截图保存到本地路径，LLM 可根据截图内容分析页面布局，随后使用 browser_click 的坐标模式进行精准点击。',
  ...
  tools.push({
    name: 'browser_click',
    label: '点击',
    description: '在页面中执行点击操作。支持两种模式：(1) 坐标模式 - 穿透 iframe/Shadow DOM/跨域框架，推荐配合截图使用；(2) 选择器模式 - 使用 CSS 选择器精准定位元素。',
    ...
    parameters: {
      ...
      properties: {
        ...
        selector: { type: 'string', description: 'CSS 选择器。与 x/y 互斥，二选一。' },
    ...
    execute: async (params: Record<string, unknown>) => {
      ...
      if (x == null && !sel) throw new Error('请提供 x/y 坐标或 CSS selector 参数，二者选一。')
      requirePage()
      ...
      if (x != null && y != null) {
        ...
        return toolResult(`已在坐标 (${x}, ${y}) 处点击。`, "browser_click", recordUsage)
      }
      await browser.clickSelector(sel!)
      return toolResult(`已点击元素 "${sel}"。`, "browser_click", recordUsage)
    },
  })
  ...
  tools.push({
    name: 'browser_type',
    label: '输入文本',
    description: '在页面中输入文本。可指定 CSS 选择器定位输入框，留空则在当前焦点元素中输入。',
    ...
    execute: async (params: Record<string, unknown>) => {
      ...
      const detail = params.selector ? `向 "${params.selector}" 输入了文本` : '在当前焦点元素输入了文本'
      return toolResult(`${detail}（${(params.text as string).length} 字符）。`, "browser_type", recordUsage)
    },
  })
  ...
  tools.push({
    ...
    parameters: {
      ...
      properties: {
        ...
        amount: { type: 'number', description: '滚动像素数，为空则滚动一个视口高度' },
      },
    },
    execute: async (params: Record<string, unknown>) => {
      requirePage()
      ...
      return toolResult(`页面已${dir === 'up' ? '向上' : '向下'}滚动。`, "browser_scroll", recordUsage)
    },
  })
  ...
  tools.push({
    ...
    label: '提取内容',
    description: '提取当前页面的文本内容。可通过 CSS 选择器提取页面特定区域的内容，留空则提取整个页面的可见文本。',
    ...
    parameters: {
      type: 'object',
      properties: {
        selector: { type: 'string', description: 'CSS 选择器，提取特定元素内的文本' },
      },
    },
    execute: async (params: Record<string, unknown>) => {
      requirePage()
  ...
  tools.push({
    ...
    label: '执行 JavaScript',
    description: '在浏览器页面中执行任意 JavaScript 代码，返回执行结果。用于高级 DOM 操作、数据提取、页面状态检查等。',
  ...
  tools.push({
    name: 'browser_find',
    label: 'Shadow DOM 定位',
    description: '在文档与所有 Shadow DOM 深层查找首个匹配 selector 的元素，返回其中心坐标与文本摘要。',
    ...
    parameters: {
      type: 'object',
      properties: {
        selector: { type: 'string', description: '要查找的 CSS 选择器（穿透 Shadow DOM）' },
      },
      required: ['selector'],
    },
    execute: async (params: Record<string, unknown>) => {
      ...
      if (!found) return toolResult(`未在文档或 Shadow DOM 中找到匹配 ${params.selector} 的元素。`, 'browser_find', recordUsage)
  ...
  tools.push({
    ...
    label: '等待元素/网络',
    description: '等待页面元素出现或网络空闲。selector 为空时改为等待网络空闲。超时不抛错，返回是否命中。',
    promptSnippet: '等待元素出现或网络空闲',
    promptGuidelines: [
      '页面加载慢或点击后元素未出现时先用本工具等待，再截图/点击',
      '命中返回 true，超时返回 false（不抛错）',
    ],
    parameters: {
      type: 'object',
      properties: {
        selector: { type: 'string', description: '要等待的 CSS 选择器。为空时改为等待网络空闲。' },
        state: { type: 'string', enum: ['visible', 'attached', 'hidden', 'detached'], description: '等待的状态，默认 visible' },
    ...
    execute: async (params: Record<string, unknown>) => {
      ...
      if (r.found) {
        const what = params.selector ? `元素 ${params.selector} 已就绪` : (r.marker === 'networkidle' ? '页面已加载（网络空闲）' : '页面已加载')
        return toolResult(`等待成功：${what}`, 'browser_wait_for', recordUsage)
      }
      const what = params.selector ? `元素 ${params.selector} 未在 ${t}ms 内达到 ${params.state ?? 'visible'}` : '页面未在超时内达到 networkidle'
  ...
  tools.push({
    ...
    label: '网络请求日志',
    description: '查询浏览器记录的 HTTP 请求，可按 URL/方法/类型过滤。clear=true 可清空日志重新开始记录。',
    ...
    parameters: {
      type: 'object',
      properties: {
        url_pattern: { type: 'string', description: 'URL 过滤（正则或子串）' },
        method: { type: 'string', description: '请求方法过滤，如 GET / POST' },
        resource_type: { type: 'string', description: '资源类型过滤，如 fetch / xhr / document / image' },
        limit: { type: 'number', description: '返回条数上限，默认 100' },
        clear: { type: 'boolean', description: 'true 时清空日志并返回' },
      },
    },
    execute: async (params: Record<string, unknown>) => {
      requirePage()
      ...
      const entries = browser.getNetwork(
        { urlPattern: params.url_pattern as string | undefined, method: params.method as string | undefined, type: params.resource_type as string | undefined },
        params.limit as number | undefined,
      )
      if (entries.length === 0) return toolResult('未捕获到匹配的网络请求。', 'browser_network', recordUsage)
      const lines = entries.map(e => `[${new Date(e.timestamp).toLocaleTimeString()}] ${e.status ?? '…'} ${e.method} ${e.type} ${e.url}`)
  ...
  tools.push({
    ...
    label: '选择下拉选项',
    description: '在 <select> 下拉框中选择一个选项。默认按 value 匹配，by_label=true 时按可见文本匹配。',
    promptSnippet: '选择下拉框中的选项',
    parameters: {
      type: 'object',
      properties: {
        selector: { type: 'string', description: '下拉框 <select> 的 CSS 选择器' },
        value: { type: 'string', description: '要选的选项 value（或 by_label=true 时的可见文本）' },
        by_label: { type: 'boolean', description: '是否按可见文本匹配，默认 false' },
      },
      required: ['selector', 'value'],
    },
    execute: async (params: Record<string, unknown>) => {
      requirePage()
      await browser.selectOption(params.selector as string, params.value as string, params.by_label as boolean | undefined)
      return toolResult(`已选择下拉框 ${params.selector} 的选项：${params.value}`, 'browser_select_option', recordUsage)
    },
  })
  ...
  tools.push({
    name: 'browser_dialog',
    label: '设置弹窗策略',
    description: '设置页面 JavaScript 弹窗处理策略。默认 dismiss（自动取消）。accept=自动确认；input=以 text 填入 prompt 并确认。只传 mode 为空时，返回最近一次捕获的弹窗文本。',
    promptSnippet: '设置或查询页面对话框处理策略',
    parameters: {
      type: 'object',
      properties: {
        mode: { type: 'string', enum: ['accept', 'dismiss', 'input'], description: '处理策略。省略时仅返回最近弹窗文本' },
        text: { type: 'string', description: 'mode=input 时填入 prompt 的文本' },
      },
    },
    execute: async (params: Record<string, unknown>) => {
      requirePage()
  ...
  tools.push({
    ...
    label: '管理/查询下载',
    description: '查询浏览器会话中已触发并保存的下载文件。可指定 dir 更改下载目录。',
    ...
    parameters: {
      type: 'object',
      properties: {
        dir: { type: 'string', description: '设置/更改下载保存目录（可选）' },
      },
    },
    execute: async (params: Record<string, unknown>) => {
      requirePage()
      const files = browser.downloads(params.dir as string | undefined)
      if (files.length === 0) return toolResult('尚未捕获到下载。', 'browser_download', recordUsage)
  ...
  tools.push({
    name: 'browser_upload',
    label: '上传文件',
    description: '向页面的 <input type="file"> 选择器设置要上传的本地文件路径。',
  ...
  tools.push({
    ...
    label: '查看/设置 Cookie',
    description: '查看或设置页面的 cookie。action=get 返回当前域 cookie；action=set 需提供 url/name/value 新增一个 cookie。',
    promptSnippet: '查看或设置页面 Cookie',
    parameters: {
      type: 'object',
      properties: {
        action: { type: 'string', enum: ['get', 'set'], description: 'get=读取；set=新增', default: 'get' },
        url: { type: 'string', description: 'action=set 时必填：cookie 所属 URL' },
        name: { type: 'string', description: 'action=set 时必填：cookie 名' },
        value: { type: 'string', description: 'action=set 时必填：cookie 值' },
      },
      required: ['action'],
    },
    execute: async (params: Record<string, unknown>) => {
      ...
      if (action === 'set') {
        if (!params.url || !params.name) return toolResult('browser_cookies action=set 需要 url、name、value。', 'browser_cookies', recordUsage)
  ...
  tools.push({
    name: 'browser_pdf',
    label: '导出 PDF',
    description: '将当前页面打印为 PDF 并保存到本地路径，返回文件路径。',
    promptSnippet: '把当前页导出为 PDF',
    parameters: {
      type: 'object',
      properties: {
        path: { type: 'string', description: 'PDF 保存路径（可选）' },
      },
    },
    execute: async (params: Record<string, unknown>) => {
  ...
  tools.push({
    name: 'browser_help',
    label: '浏览器交互手册',
    description: '查询 pi-browser 的 Web 交互手册（坐标转换、Shadow DOM、下拉框、弹窗、下载/上传、网络捕获、滚动、iframe、等待、cookie、截图），返回处理对应浏览器机制的实操要领。',
    ...
    parameters: {
      type: 'object',
      properties: {
        topic: { type: 'string', description: '要查询的主题（可选）' },
      },
    },
    execute: async (params: Record<string, unknown>) => {
      ...
      const INTERACTION_DOC = join(REFERENCES_DIR, 'interaction.md')
      let doc: string
      try {
      ...
      if (topic) {
        ...
        return toolResult(`未找到主题「${topic}」。`, 'browser_help', recordUsage)
      }
  ...
  tools.push({
    ...
    execute: async () => {
      await browser.close()
      return toolResult('浏览器实例已关闭，资源已释放。', "browser_close", recordUsage)
    },
  })
