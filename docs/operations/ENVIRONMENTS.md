...
| 版本 | v1.0 |
| 更新日期 | 2026-09-12 |
| 适用范围 | 多环境配置、记忆同步、配置管理 |
| 相关文档 | [TERMUX-DEV-NOTES.md](./TERMUX-DEV-NOTES.md), [AGENTS-DETAILS.md](../development/AGENTS-DETAILS.md) |
...
- [一、环境识别](#一环境识别)
- [二、记忆环境标签](#二记忆环境标签p0-已实施)
- [三、运行时数据冲突消解](#三运行时数据冲突消解p1)
- [四、配置层](#四配置层每环境独立)
- [五、常见环境差异坑](#五常见环境差异坑)
...
- **打标规则**：
  ...
  - 已有条目回填已按关键词完成；按环境条数实时统计（快照数字会过期，勿引用死数字）：`python3 -c "import json;from collections import Counter as C;es=json.load(open('memory/entries.json'))['entries'];print(C(x for e in es for x in (e['environments'] if isinstance(e.get('environments'),list) else [e.get('environments','all')])))"`（2026-08-28 快照：all 312 / wsl2 175 / termux 141 / linux 7 / windows 4；当前值可能已变化，运行命令刷新），误标时编辑 `memory/entries.json` 修正
...
| `pi-link.json` | **每环境独立**（gitignored） | pi-link 多设备互联设备清单（host/user/port/selfName/allowUnattended）；运行时文件 `pi-link-active.json`/`pi-link-state.json`/`pi-link-outbox.json` 同理不入库。多设备接入流程见 `.pi/extensions/pi-link/README.md` |
