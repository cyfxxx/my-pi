...
> 多环境使用总览见 `docs/operations/ENVIRONMENTS.md`。

本机环境实测经验（2026-08 汇总，my-pi voice 功能录音链路反复踩坑总结）。适用于在 Termux（PRoot 容器）内开发涉及 Android 系统 API 的功能。
...
- [四、终端/输入](#四终端输入)
- [五、网络](#五网络)
- [六、sshd 与 link 功能](#六sshd-与-link-功能多设备互联)
...
- 共享存储目录需先创建且可写（在 `/storage/emulated/0/` 下自建录音目录），并确认 Termux 有存储权限
...
- **git 推送/拉取走 SSH over 443（免代理免 PAT）**：Termux 侧把 remote 配成 `ssh://git@ssh.github.com:443/<owner>/<repo>.git`（GitHub 的 443 端口 SSH 入口），认证用 `~/.ssh/id_ed25519`。本机实测：`github.com:443` 直连不可用（全 IP 段）、`api.github.com` 可直连、开 v2ray 代理反而会挂——**不要改回 HTTPS 或开代理**
- **git push 被拒（non-fast-forward）时**：先 `git pull --rebase origin main`；被跟踪的共享文件冲突统一保留远程版本（`git checkout --theirs <file>`），本机确实需要的改动从备份手工重放后再 `git add`（详见 [ENVIRONMENTS.md](./ENVIRONMENTS.md) §3.2）

## sshd 与 link 功能（多设备互联）
...
- **语言自动检测不可靠**：base 模型对中文短句/噪声环境易误判英文，固定语言参数（whisper 的 `--language zh`）解决
- **模型选择**：base 识别率一般，中文建议 small；速度取决于 CPU，Android PRoot 下明显慢于实时
- my-pi 侧接入：voice 功能（`custom/features/voice/`）配置中 `sttProvider` 默认为 `whisper`；本地转写工具与模型规格记录在 `packs/media-toolkit/references/specialized-tools.md`
