...
4. **每环境独立项**：`portable/config/{auth.json,models.json,models-store.json,modes.json,trust.json,pi-link-*.json,scheduled-seeds.json}`、`portable/sessions/`、`portable/extensions/` 均为每环境独立数据（gitignore），默认既不进 git 同步也不进归档。跨机迁移三选一：① `pi-backup create --with-auth` 打包 → restore；② scp 直接传；③ 新设备手动重建。
...
7. **技能目录**：技能位于 `portable/config/skills/<name>/SKILL.md`（pi 从 `agentDir/skills` 自动发现，`agentDir` = `portable/config`）。`portable/skills/` 是占位目录，**pi 不读取**，无需备份。`packs/` 是按需读取的外部技能包（不注入系统提示词），同样建议纳入归档。
