...
>
> my-pi 迁移（2026-09-20）：下文核心包路径已指向 `<root>/vendor/pi/packages/coding-agent`，
> 备份文件位于该包的 `dist/` 下；npm 扩展相关条目在 my-pi 中不存在，保留作历史参考。
...
1. 仓库内 vendored 上游 `<root>/vendor/pi/packages/coding-agent`（my-pi 最高优先级）
2. 常见全局安装路径 (`/usr/lib`, `/usr/local/lib`)
3. `npm root -g` 输出
4. `require.resolve()` 模块解析
5. 家目录下 node_modules 搜索
...
```bash
# 查看备份文件
ls -la <root>/vendor/pi/packages/coding-agent/dist/modes/interactive/components/*.bak.*
