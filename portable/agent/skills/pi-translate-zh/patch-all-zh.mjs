 ...
 *
 * my-pi 迁移说明：
 *   - pi 核心包解析优先使用脚本位置上溯得到的仓库根 + vendor/pi/packages/coding-agent
 *   - 可选 npm 扩展（plannotator / pi-lens / pi-markdown-preview / pi-subagents /
 *     rpiv-todo / context-mode）在 my-pi 中不存在，对应区段通过 missingAll 机制安全跳过
 *
 * 用法：node patch-all-zh.mjs
 * 每次重新构建 vendor/pi（bash scripts/sync-upstream.sh + bash scripts/build.sh）后执行一次。
 *
 * 特性：
 *   - 优先解析仓库内 vendor/pi/packages/coding-agent，兼容 npm global / nvm / 自定义 prefix
 *   - 修改前自动备份原文件至 .bak.时间戳
 *   - 增量安全：已翻译的字符串不会被重复替换
 *   - 结构清晰的翻译区段
 */
...
function resolvePiPath() {
	const candidates = [
		// 最高优先级：my-pi 仓库内 vendored 上游源码（<root>/vendor/pi/packages/coding-agent）
		join(REPO_ROOT, "vendor", "pi", "packages", "coding-agent"),
		// 最常见全局安装路径
...
/** DRY_RUN 感知的写回：干跑时只计数不落盘（写前自动备份） */
...
sections.push(() => {
	...
	if (existsSync(INTERACTIVE_MODE)) {
		...
		if (changed > 0) {
			writeTarget(INTERACTIVE_MODE, content);
			n += changed;
		}
	}
	return `交互模式消息 (${n} 条)`;
...
sections.push(() => {
	...
	for (const dir of dirs) {
		...
		writeTarget(zhPath, JSON.stringify(zhTodo, null, 2) + "\n");
		count++;
	}
...
sections.push(() => {
	...
	if (changed > 0) {
		writeTarget(EXT, modified);
		n = changed;
	}
	return `subagents 扩展 (${n} 项)`;
});

// ---- [28] context-mode pi-extension 命令描述 ----
sections.push(() => {
...
if (skipped > 0) {
	console.log(`\n（${skipped} 节跳过：对应扩展在 my-pi 中不存在，属正常情况）`);
}
