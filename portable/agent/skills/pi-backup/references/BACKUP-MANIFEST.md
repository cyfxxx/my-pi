...
> **技能目录收录方式**：`portable/agent/skills/` 下的技能随仓库跟踪（`.gitignore` 白名单）；若本机新装的技能尚未入库，`create` 归档仍会按"未被 gitignore 排除"原则收录。
...
| 分组 | 相对路径 | 说明 |
|------|----------|------|
| auth | `portable/agent/auth.json` | API 密钥。**默认不包含**，需 `--with-auth` 确认。包含后应提醒用户注意安全。 |
| 模型配置 | `portable/agent/models.json`、`portable/agent/models-store.json` | provider/模型定义（含 provider 密钥，机器特定）。**默认不包含**，随 `--with-auth` 一并收录；否则新设备需 scp 或手动重建。 |
| 模式/信任 | `portable/agent/modes.json`、`portable/agent/trust.json` | 模式定义与项目信任设置（每环境独立）。**默认不包含**，随 `--with-auth` 一并收录。 |
| 会话历史 | `portable/agent/sessions/` | 对话历史。**默认不包含**，需 `--include-sessions`。 |
