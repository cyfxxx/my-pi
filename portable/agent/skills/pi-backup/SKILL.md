...
对 my-pi 仓库（`/root/my-pi`）进行打包备份与恢复：`custom/`、`scripts/`、`patches/`、`packs/`、`portable/agent` 中跟踪的配置文件、`portable/memory` 笔记数据、`portable/agent/skills/` 技能（`docs/` 存在时一并收录）。支持两种模式：
...
1. **敏感数据**：`portable/agent/auth.json`（API 密钥）与 `models.json` / `models-store.json`（provider 密钥）默认不包含在备份中。`git sync` 时 `.gitignore` 会自动排除它们——但仍建议定期确认 `git ls-files portable/agent` 中不含这些文件，防止意外追踪。
2. **重启生效**：恢复或克隆后必须重新运行 `./my-pi.sh` 才能加载更新后的配置。
3. **恢复前快照**：每次 `restore` 操作会自动创建 `~/pi-backups/pre-restore-{timestamp}.tar.gz`，可用于回滚。
4. **每环境独立项**：`portable/agent/{auth.json,models.json,models-store.json,trust.json,pi-link-*.json}`、`portable/agent/sessions/`、`portable/agent/extensions/` 均为每环境独立数据（gitignore），默认既不进 git 同步也不进归档（`modes.json`、`scheduled-seeds.json` 已入白名单随仓库分发）。跨机迁移三选一：① `pi-backup create --with-auth` 打包 → restore；② scp 直接传；③ 新设备手动重建。
...
8. **记忆数据**：`portable/memory/` 整体被 gitignore（仅保留 `.gitkeep`），包含记忆笔记 `notes.json`（由 `custom/features/memory/store/notes.ts` 读写）与工具输出归档 `tool-outputs/`——**git 同步不带走**，换机保留记忆必须用 `create` 归档。`tool-outputs/` 属可再生的运行时归档，默认排除。
